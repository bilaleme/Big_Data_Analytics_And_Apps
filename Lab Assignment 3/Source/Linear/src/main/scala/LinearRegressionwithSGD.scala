...
//    System.setProperty("hadoop.home.dir","C:\\Users\\Manikanta\\Documents\\UMKC Subjects\\PB\\hadoopforspark");
    ...
    val data = sc.textFile("data\\ipsa1.data")
...
//    val model = LinearRegressionWithSGD.train(training, numIterations, stepSize)
    ...
    val model = new LinearRegressionWithSGD().setIntercept(true)
    model.optimizer.setStepSize(0.01)
    model.optimizer.setNumIterations(numIterations)
    val model1 = model.run(training)
    ...
    println("Speed Intercept (Y-Intercept) : "+model1.intercept)
    println("Movement (Slope) : "+model1.weights)
    // Save and load model
    model1.save(sc, "data\\LinearRegressionWithSGDModel")
