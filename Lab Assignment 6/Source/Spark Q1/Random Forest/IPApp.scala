...
//  val IMAGE_CATEGORIES = List("rice", "tempura", "toast", "bibimap", "sushi", "spaghetti", "sausage", "oden", "omelet", "jiaozi")
  val IMAGE_CATEGORIES = List("cars","airplanes","ships")
...
//    val nbModel = DecisionTreeModel.load(sc, IPSettings.DECISION_TREE_PATH)
  ...
  def generateRandomForestModel(sc: SparkContext): Unit = {
    ...
    val parsedData = data.map { line =>
      ...
      LabeledPoint(Math.abs(parts(0).toDouble), Vectors.dense(parts(1).split(' ').map(_.toDouble)))
    }
  ...
  def generateDecisionTreeModel(sc: SparkContext):Unit = {
    if (Files.exists(Paths.get(IPSettings.DECISION_TREE_PATH))) {
      println(s"${IPSettings.DECISION_TREE_PATH} Decision Tree Model already exists therefore skipping it's formation")
      return
    }
    ...
    val predictionAndLabel = test.map { point =>
      val prediction = model.predict(point.features)
      (Math.abs(point.label), prediction)
    }
...
//      println(s"Predicting test image : " + cat + " as " + IMAGE_CATEGORIES(predictedClass.toInt))
      results = results + "Predicting test image : " + cat + " as " + IMAGE_CATEGORIES(predictedClass.toInt)+"\n"
